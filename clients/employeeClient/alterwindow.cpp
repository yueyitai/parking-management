...
void alterwindow::on_alterFindButton_clicked()
{
    ...
    //ui->alterAgeEdit->setText(QString("{%1}").arg(alterjson["age"].toInt()));
...
{
    int state = 0;
    if(ui->alterStateTimeEdit->text() == "在职") state = 1;
    else if (ui->alterStateTimeEdit->text() == "离职") state = 0;
    //创建一个json对象
    QJsonObject jsonObject;
    //获取文本框信息
    jsonObject["id"]            = ui->alterIdEdit->text().toInt();
    ...
    jsonObject["age"]           = ui->alterAgeEdit->text().toInt();
